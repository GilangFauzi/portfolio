    ...
    <section id="contact" className="py-24 sm:py-28 px-4 sm:px-6 lg:px-8 bg-surface-alt relative">
      <div className="max-w-4xl mx-auto text-center" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} transition={{ duration: 0.5 }}>
          <SectionHeading number={7} title="Get In" highlight="Touch" />
        </motion.div>
        <motion.p initial={{ opacity: 0, y: 10 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 10 }} transition={{ duration: 0.4, delay: 0.15 }} className="text-text-secondary max-w-lg mx-auto mb-10 leading-relaxed">
          Saya terbuka untuk peluang kolaborasi, freelance, atau posisi full-time di bidang software engineering, web development, dan IoT. Jangan ragu untuk menghubungi saya!
        </motion.p>
        ...
        <motion.p initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : { opacity: 0 }} transition={{ duration: 0.4, delay: 0.4 }} className="text-text-muted text-sm">&copy; {new Date().getFullYear()} Gilang Fauzi. Built with React &amp; Tailwind CSS.</motion.p>
      </div>
    </section>
  );
}
