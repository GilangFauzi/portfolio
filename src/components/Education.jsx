...
const education = [
  { institution: "Universitas Pamulang", degree: "S1 Teknik Informatika", period: { id: "11 Oktober 2018 – 13 Oktober 2022", en: "October 11, 2018 – October 13, 2022" }, gpa: "3.36 / 4.0", thesis: { id: "Implementasi Alat Penyiraman Tanaman Otomatis Berbasis Internet of Things menggunakan Metode Fuzzy Logic pada Kebun Artawi Flora.", en: "Implementation of Automatic Plant Watering Device Based on Internet of Things using Fuzzy Logic Method at Artawi Flora Garden." } },
];
const organizations = [
    ...
    <section id="education" className="py-24 sm:py-28 px-4 sm:px-6 lg:px-8 bg-surface relative">
      <div className="max-w-4xl mx-auto" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} transition={{ duration: 0.5 }}>
          <SectionHeading number={4} title={t.education.title} highlight={t.education.highlight} />
        </motion.div>
        <div className="mb-14">
          ...
          {education.map((edu, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} transition={{ duration: 0.5, delay: 0.15 }} whileHover={{ borderColor: "rgba(34, 211, 238, 0.3)" }} className="bg-surface-card border border-border rounded-xl p-6 backdrop-blur-sm">
              <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                ...
                <span className="text-xs font-mono text-cyan-400 bg-cyan-400/10 px-3 py-1 rounded-full">{edu.period[lang]}</span>
              </div>
              ...
              <p className="text-text-muted text-sm leading-relaxed"><span className="text-text-secondary font-medium">{t.education.thesis}:</span> {edu.thesis[lang]}</p>
            </motion.div>
          ))}
        </div>
        <div>
